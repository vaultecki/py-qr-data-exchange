...
class CryptoError(Exception):
    """General crypto errors thrown by this module."""
    pass
...
class CryptoUtils:
    ...
    def generate_salt() -> bytes:
        ...
        Returns:
            salt (bytes): salt with length nacl.pwhash.argon2i.SALTBYTES
        """
    ...
    def validate_salt(salt: bytes) -> None:
        ...
        if len(salt) != nacl.pwhash.argon2i.SALTBYTES:
            raise ValueError(f"Salt must be {nacl.pwhash.argon2i.SALTBYTES} bytes long.")
    ...
    def derive_key(password: str, salt: bytes,
        ...
        Args:
            password: The user password as str.
            salt: Salt (bytes) with length nacl.pwhash.argon2i.SALTBYTES.
            opslimit: Optional. Argon2 opslimit (tuning).
            memlimit: Optional. Argon2 memlimit (tuning).

        Returns:
            key (bytes): Key with length nacl.secret.SecretBox.KEY_SIZE

        Raises:
            ValueError: for invalid arguments.
            CryptoError: for KDF errors.
        """
    ...
    def encrypt(data: bytes, key: bytes) -> bytes:
        """
        Encrypts raw bytes with secretbox(key).

        Args:
            data: Raw bytes (e.g. from file)
            key: Key as generated by derive_key.

        Returns:
            bytes: the encrypted message generated by SecretBox (nonce + ciphertext)
    ...
    def decrypt(encrypted_data: bytes, key: bytes) -> bytes:
        """
        Decrypts data that was created with `encrypt`.
        ...
        Raises:
            CryptoError for problems (incl. WrongKey)
        """
    ...
    def encode_base64(data: bytes) -> str:
        """
        Encodes bytes -> base64 ascii string.
        """
        if not isinstance(data, (bytes, bytearray)):
